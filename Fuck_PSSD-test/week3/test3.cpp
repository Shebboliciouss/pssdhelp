...
// class QuickSums{
// public:
//     std::vector<std::pair<int, int>> generateSums(const std::string& input, int startIndex, int currentSum, int plusCount, int targetSum) {
...
//     for (int i = 1; i <= input.length() - startIndex; ++i) {
//         std::string numStr = input.substr(startIndex, i);
//         int num = std::stoi(numStr);
        
//         std::vector<std::pair<int, int>> subResults = generateSums(input, startIndex + i, currentSum + num, plusCount + 1, targetSum);
...
//         int ans = -1;
//         std::vector<std::pair<int, int>> results = generateSums(numbers, 0, 0, 0, targetSum);
//         int n = results.size(); 
//         for (int i = n-1; i >= 0; i--) {
