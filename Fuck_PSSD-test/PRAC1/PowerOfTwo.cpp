...
//     // Create a map to keep track of the number of ways to represent each value
...
//         // For each coin value, update the number of ways to make various totals
//         for (auto& [value, count] : dp) {
//             long long new_value = value + coin;
//             if (new_value <= N) {
//                 // Two ways to represent new_value: either using the coin or not
//                 new_dp[new_value] += count;
//             }
//         }
...
//To use the dynamic programming that to count with the number of ways that to represent N
vector<long> dp(N+1,0);
dp[0]=1;
for(long coin :coins)
{
